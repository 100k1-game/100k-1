...
createApp({
    setup() {
        ...
        const validateKey = () => {
            ...
            const lb = LobbyKeys.validate(key);
            if (!lb) { error.value = 'Неверный ключ доступа'; return; }
            keyValidated.value = true;
            sync.saveKey(LobbyKeys.getKey(lb.id));
            
            // Пытаемся открыть сразу, если действие инициировано пользователем
            openAllPages();
        };
        ...
        const openAllPages = () => {
            ...
            window.open(APP_CONFIG.pageUrl('host.html', { key }), '_blank');
            window.open(APP_CONFIG.pageUrl('editor.html', { key }), '_blank');
            window.location.href = APP_CONFIG.pageUrl('game.html', { key });
        };

        return { keyValidated, keyInput, error, formatInput, validateKey, openPage, openAllPages };
    }
}).mount('#app');
